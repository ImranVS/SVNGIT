...
BSTR GetServerStat(char* pServerName, char* pFacility, char* pStat){
	...
	try{
		...
		if (nError = NSFGetServerStats(pServerName, pFacility, pStat, &hStatBuffer, &dwStatBufferLen1) == NOERROR)
		{
			//'Lock memory to get pointer to buffer
			char* ptrBuffer = (char *) OSLockObject(hStatBuffer);

			// Copy results to buffer as we will release the locked memory but still need to return results to caller
			char szStatValue[5000];						// will only process max 5000 chars from Domino server
			memcpy(szStatValue, ptrBuffer, (WORD) dwStatBufferLen1);
			szStatValue[dwStatBufferLen1] = '\0';

			// Copy buffer contents into CString for return to caller
			csResults = szStatValue;
