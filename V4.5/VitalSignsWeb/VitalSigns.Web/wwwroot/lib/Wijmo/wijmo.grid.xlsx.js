        ...
        (function (xlsx) {
            ...
            var FlexGridXlsxConverter = (function () {
                ...
                FlexGridXlsxConverter._parseFlexGridRowToSheetRow = function (panel, workbookRow, rowIndex, startColIndex, columnSettings, includeCellStyles, fakeCell, isGroupRow, groupLevel) {
                    var flex, row, columnSetting, ci, format, val, unformattedVal, groupHeader, isFormula, formula, cellIndex, cellStyle, mergedCells, rowSpan, colSpan, sheetInfo, valIsDate, isCommonRow = false, propName, col, isStartMergedCell;
                    flex = panel.grid;
                    sheetInfo = flex['wj_sheetInfo'];
                    ...
                    for (ci = 0; ci < panel.columns.length; ci++) {
                        ...
                        columnSetting = columnSettings[ci + startColIndex];
                        ...
                        if (panel === flex.cells && isGroupRow && row['hasChildren'] && ci === flex.columns.firstVisibleIndex) {
                            ...
                            if (val) {
                                ...
                                propName = row.dataItem && row.dataItem.groupDescription ? row.dataItem.groupDescription.propertyName : '';
                                col = flex.columns.getColumn(propName);
                                if (col) {
                                    propName = col.header;
                                }
                                groupHeader = propName ? propName + ': ' + row.dataItem.name + ' (' + row.dataItem.items.length + ' items)' : '';
                            }
