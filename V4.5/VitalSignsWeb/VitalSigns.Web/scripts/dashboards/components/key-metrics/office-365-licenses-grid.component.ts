...
@Component({
    templateUrl: './app/dashboards/components/key-metrics/office-365-licenses-grid.component.html',
    providers: [
        WidgetService,
        HttpModule,
        RESTService,
...
export class Office365LicensesGrid implements OnInit {
    ...
    loadData() {
        this.isLoading = true;
        this.service.get(`/dashboard/office_365_account_licenses`)
            .subscribe(
            (response) => {
            ...
            (error) => { this.errorMessage = <any>error; this.isLoading = true; }
            );
