...
{
    public partial class ExchangeMsgXtraRpt : DevExpress.XtraReports.UI.XtraReport
    {
        public ExchangeMsgXtraRpt()
        {
            InitializeComponent();
        }

        private void ExchangeMsgXtraRpt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            DataTable dt = new DataTable();
            dt = VSWebBL.ReportsBL.ReportsBL.Ins.GetExchangeMailSentCount(this.ServerName.Value.ToString(), "Mail_SentCount",
                this.DateFrom.Value.ToString(),this.DateTo.Value.ToString(),this.RptType.Value.ToString());
            Series series = null;
            string srvName = "";
