...
{
	public partial class OverallHealth1 : System.Web.UI.Page
	{
		...
		public void KeyMetrics(string ServerLoc)
		{
			...
			foreach (DataRow row in dt.Rows)
			{
				...
				{
                    //3/17/2016 NS modified for VSPLUS-2719
		...
		protected void Save()
		{
			try
			{
				//12/12/2013 NS modified
				ASPxCheckBoxList checkbxlist = (ASPxCheckBoxList)PopupControl2.FindControl("checkBoxList1");
				var list = checkbxlist.SelectedItems.Count;
				//string set=checkbxlist.SelectedItem.Value
				string str1 = null;
				
				
				for (int i = 0; i < checkbxlist.SelectedValues.Count; i++)
				{
					if (checkbxlist.SelectedItems[i].Selected == true)
					{
				...
				{
					Session["FilterByValue"] = "null";
                    Session["MyServerTypes"] = null;
                    Session["MyCategory"] = null;
                    Session["MyServerLocations"] = null;
				}
		...
		public string GetLink(object status, object typeloc, object isBtn)
		{
			string Status = status.ToString();
			string TypeLoc = typeloc.ToString();
            if (TypeLoc == "Office 365")
            {
                TypeLoc = "Office365";
            }
            //4/27/2015 NS modified for VSPLUS-2901
			if (Status == "ALL")
            {
                if (Convert.ToBoolean(isBtn))
                {
                    return "location.href='../Dashboard/DeviceTypeList.aspx?typeloc=" + TypeLoc + "'";
                }
                else
                {
            ...
            if (Convert.ToBoolean(isBtn))
            {
                return "location.href='../Dashboard/DeviceTypeList.aspx?status=" + status + "&typeloc=" + TypeLoc + "'";
            }
            else
            {
		...
		protected void ASPxMenu1_ItemClick(object source, DevExpress.Web.MenuItemEventArgs e)
		{
            ...
            else if (e.Item.Name == "ViewbyType")
            {
                Session["FilterByValue"] = "null";
                VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("ViewBy", "ServerType", Convert.ToInt32(Session["UserID"]));
                Session["ViewBy"] = "ServerType";//lnkViewByServerType.Text;
            ...
            else if (e.Item.Name == "ViewbyLocation")
            {
                Session["FilterByValue"] = "null";
                VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("ViewBy", "Location", Convert.ToInt32(Session["UserID"]));
                //VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("FilterBy", "null", Convert.ToInt32(Session["UserID"]));
		...
		protected void PopupControl2_Load()

		{
			DataTable dt = VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.GetSelctedData(Convert.ToInt32(Session["UserID"]));
			if (dt.Rows.Count > 0)
			{
				string filterby = dt.Rows[0]["PreferenceValue"].ToString();
				string[] rowValueItems = filterby.Split(',');
				...
				checkbxlist.DataBind();
				foreach (ListEditItem item in checkbxlist.Items)
				{
					int i = checkbxlist.Items.Count;
					...
					string svalue = "'" + value + "'";
					for (int j = 0; j < rowValueItemsAsList.Count; j++)
					{
						if (rowValueItemsAsList[j].ToString() == svalue)
						{
							item.Selected = true;
						}
						//else
