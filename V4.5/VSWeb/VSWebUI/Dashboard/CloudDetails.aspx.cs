...
{
    public partial class CloudDetails : System.Web.UI.Page
    {
            ...
            if (!IsPostBack && !IsCallback)
            {
                ...
                if (Request.QueryString["Name"] != "" && Request.QueryString["Name"] != null)
                {
                    ...
                    //CpuHyperLink.NavigateUrl = "DominoServerStatisticsDetail.aspx?name=Cpu&server=" + servernamelbl.Text;
                                       PerfrmHyperLink.NavigateUrl = "CloudDetails.aspx?name=Perfm&server=" + servernamelbl.Text;
                }
        ...
        public DataTable SetGraph(string paramGraph, string DeviceName)
        {
            try
            {
				...
				DataTable dt = VSWebBL.DashboardBL.CloudDetailsBL.Ins.SetGraphCloud(paramGraph, DeviceName);
                ...
                DataTable dt1 = VSWebBL.DashboardBL.CloudDetailsBL.Ins.ResponseThreshold(DeviceName);
                     if (dt1.Rows.Count > 0)
                     {
        ...
        protected void ASPxMenu1_ItemClick(object source, DevExpress.Web.MenuItemEventArgs e)
        {
            ...
            else if (e.Item.Name == "ScanItem")
            {
                try
                {
                    ...
                    else if (Request.QueryString["Type"] == "Cloud")
                    {
                        bl = VSWebBL.SettingBL.SettingsBL.Ins.UpdateSvalue("ScanCloudASAP", StatusObj.Name, VSWeb.Constants.Constants.SysString);
                    }
            ...
            else if (e.Item.Name == "EditConfigItem")
            {
                ...
                if (Request.QueryString["Name"] != "" && Request.QueryString["Name"] != null)
                {
                    id = (VSWebBL.SecurityBL.ServersBL.Ins.GetCloudIDbyDeviceName(Request.QueryString["Name"].ToString())).ToString();
					//id = (VSWebBL.SecurityBL.ServersBL.Ins.GetServerIDbyServerName(Request.QueryString["Name"].ToString())).ToString();
					Response.Redirect("~/Configurator/CloudApplicationsServerProperties.aspx?ID=" + id, false);//Mukund, 05Aug14, VSPLUS-844:Page redirect on callback
                    Context.ApplicationInstance.CompleteRequest();
                }
            }
