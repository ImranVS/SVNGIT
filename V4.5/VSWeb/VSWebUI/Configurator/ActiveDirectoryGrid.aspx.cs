...
{
    public partial class ActiveDirectoryGrid : System.Web.UI.Page
    {
		int UserID;
        ...
        private void FillWindowsServerGrid()
        {
            try
            {
				...
				if (Session["UserID"] != null)
				{
					...
					DSTaskSettingsDataTable = VSWebBL.ConfiguratorBL.ActiveDirectoryBAL.Ins.GetAllDatabyUser(UserID);
				}
               // DSTaskSettingsDataTable = VSWebBL.ConfiguratorBL.ActiveDirectoryBAL.Ins.GetAllData();
        ...
        protected void GenericGridView_HtmlRowCreated(object sender, DevExpress.Web.ASPxGridViewTableRowEventArgs e)
        {
            ...
            if (e.RowType == GridViewRowType.EditForm)
            {
                if (e.GetValue("ID").ToString() != " ")
                {
                    ...
                    try
                    {
                        ASPxWebControl.RedirectOnCallback("ActiveDirectoryProperties.aspx?ID=" + e.GetValue("ID") + "&name=" + e.GetValue("ServerName") + "&Cat=" + e.GetValue("ServerType") + "&Loc=" + e.GetValue("Location") + "&ipaddr=" + e.GetValue("ipaddress"));
        ...
        protected void GenericGridView_SelectionChanged(object sender, EventArgs e)
        {
            if (GenericGridView.Selection.Count > 0)
            {
                ...
                if (Type.Count > 0)
                {
                    ...
                    try
                    {
                        DevExpress.Web.ASPxWebControl.RedirectOnCallback("ActiveDirectoryProperties.aspx?ID=" + ID + "&name=" + name + "&Cat=" + category + "&Loc=" + Loc + "&ipaddr=" + ipaddr);
                        Context.ApplicationInstance.CompleteRequest();//Mukund, 05Aug14, VSPLUS-844:Page redirect on callback
