...
{
    public partial class SharePointGrid : System.Web.UI.Page
    {
		int UserID;
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["UserPreferences"] != null)
                {
                    ...
                    foreach (DataRow dr in UserPreferences.Rows)
                    {
                        ...
                        {
                            sharepointGridView.SettingsPager.PageSize = Convert.ToInt32(dr[2]);
                        }
                    }
                }
            }
        ...
        private void FillExchangeServerGrid()
        {
            try
            {
                ...
                //DSTaskSettingsDataTable = VSWebBL.ConfiguratorBL.SharePointBAL.Ins.GetAllData();
				DSTaskSettingsDataTable = VSWebBL.ConfiguratorBL.SharePointBAL.Ins.GetAllDataByUserID(UserID);
                if (DSTaskSettingsDataTable.Rows.Count > 0)
                {
                    ...
                    sharepointGridView.DataSource = DSTaskSettingsDataTable;
                    sharepointGridView.DataBind();
                }
            }
            catch (Exception ex)
            {
        ...
        protected void sharepointGridView_SelectionChanged(object sender, EventArgs e)
        {
            if (sharepointGridView.Selection.Count > 0)
            {
                System.Collections.Generic.List<object> Type = sharepointGridView.GetSelectedFieldValues("ID");
                System.Collections.Generic.List<object> Name = sharepointGridView.GetSelectedFieldValues("ServerName");
                System.Collections.Generic.List<object> Cat = sharepointGridView.GetSelectedFieldValues("ServerType");
                System.Collections.Generic.List<object> Location = sharepointGridView.GetSelectedFieldValues("Location");
                System.Collections.Generic.List<object> ipaddress = sharepointGridView.GetSelectedFieldValues("ipaddress");
                if (Type.Count > 0)
                {
                    ...
                    try
                    {
                        DevExpress.Web.ASPxWebControl.RedirectOnCallback("SharepointServer.aspx?ID=" + ID + "&name=" + name + "&Cat=" + category + "&Loc=" + Loc + "&ipaddr=" + ipaddr);
                        Context.ApplicationInstance.CompleteRequest();//Mukund, 05Aug14, VSPLUS-844:Page redirect on callback
        ...
        protected void sharepointGridView_PageSizeChanged(object sender, EventArgs e)
        {
            //ProfilesGridView.PageIndex;
            VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("MSServersGrid|MSServerGridView", sharepointGridView.SettingsPager.PageSize.ToString(), Convert.ToInt32(Session["UserID"]));
