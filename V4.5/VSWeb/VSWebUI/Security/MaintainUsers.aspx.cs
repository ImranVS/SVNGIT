...
namespace VSWebUI.Security
{
    public partial class MaintainUsers : System.Web.UI.Page
	{
        ...
        protected void UsersGridView_RowInserting(object sender, DevExpress.Web.Data.ASPxDataInsertingEventArgs e)
        {
			...
			else
			{
				...
				else
				{
                   ...
                   // Response.Write("<script>alert('" + Server.HtmlEncode(errString.ToString()) + "')</script>");

					throw new Exception("Error: " + errString + ". Check SMTP settings at Alerts - Alert Settings and then return to create a user.");


					//throw new System.InvalidOperationException("Error: " + errString + ". Check SMTP settings at Alert - Alert Settings and then return to create a user.");
					//throw new System.ArgumentException("Error: " + errString + ". Check SMTP settings at Alert - Alert Settings and then return to create a user.");
					throw new CustomExceptions("Error: " + errString + ". Check SMTP settings at Alerts - Alert Settings and then return to create a user.");
					//throw new System.InvalidOperationException("Error: " + errString + ". Check SMTP settings at Alert - Alert Settings and then return to create a user.");
					//throw new System.ArgumentException("Error: " + errString + ". Check SMTP settings at Alert - Alert Settings and then return to create a user.");
        ...
        public string Sendmail(DataRow UsersRow, string userpwd)
        {
            try
            {
                ...
                else 
                {
					...
					mailbody = "Welcome to VitalSigns.You account details updated as follows:\nLoginName: " + UsersRow["LoginName"].ToString() +  "";
                }
        ...
        private void UpdateUsersData(string Mode, DataRow UsersRow, string userpwd)
        {
            ...
            if (Mode == "Insert")
            {
                Object ReturnValue = VSWebBL.SecurityBL.UsersBL.Ins.CreateAccount(CollectDataForUsers(Mode, UsersRow, userpwd));
            }
            if (Mode == "Update")
            {
                Object ReturnValue = VSWebBL.SecurityBL.UsersBL.Ins.UpdateData(CollectDataForUsers(Mode, UsersRow, userpwd));
        ...
        private Users CollectDataForUsers(string Mode, DataRow UsersRow, string userpwd)
        {
            try
            {
				...
				UsersObject.IsFirstTimeLogin = true;
                UsersObject.cloudindex = 1;
                UsersObject.premisesindex = 0;
                UsersObject.networkindex = 1;
                UsersObject.dockindex = 0;
                UsersObject.cloudZone = "LeftZone";
                UsersObject.premisesZone = "RightZone";
                UsersObject.networkZone = "MiddleZone";
                UsersObject.DockZone = "LeftZone";
        ...
        protected void UsersGridView_RowDeleting(object sender, DevExpress.Web.Data.ASPxDataDeletingEventArgs e)
        {
            ...
            //Delete row from DB
            Object ReturnValue = VSWebBL.SecurityBL.UsersBL.Ins.DeleteData(UserObject);
        ...
        protected void bttnOK_Click(object sender, EventArgs e)
        {
            Users usersObj = new Users();
            usersObj.ID = userID;
            Object returnValue = VSWebBL.SecurityBL.UsersBL.Ins.DeleteData(usersObj);
