...
Partial Public Class VitalSignsPlusDomino
	...
	Private Sub CheckTravelerServer(ByRef myDominoServer As MonitoredItems.DominoServer)
		...
		Try
            strSQL = "Update Traveler_Status SET  HTTP_Status='" & Status & "', HTTP_Details='" & Trim(Details) & "' WHERE ServerName='" & myDominoServer.Name & "'"
		...
		Try
			If myDominoServer.Traveler_Version <> "n/a" And myDominoServer.Traveler_Version <> "" Then
				strSQL = "Update Traveler_Status SET  TravelerVersion='" & myDominoServer.Traveler_Version & "' WHERE ServerName='" & myDominoServer.Name & "'"
				WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
		...
		Try
			...
			If InStr(myDominoServer.Statistics_Traveler, "Traveler.Constrained.State") Then
				...
				WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " Traveler.Constrained.State = " & Constrained)
				If Constrained = 0 Then
					strSQL = "Update Traveler_Status SET ResourceConstraint='Pass' WHERE ServerName='" & myDominoServer.Name & "'"
					myAlert.ResetAlert("Domino", myDominoServer.Name, "Traveler Resource Constraint", "The Traveler server is not operating under a detected resource constraint.")
				ElseIf Constrained = 1 Then
					strSQL = "Update Traveler_Status SET ResourceConstraint='Fail' WHERE ServerName='" & myDominoServer.Name & "'"
					...
					myAlert.QueueAlert("Domino", myDominoServer.Name, "Traveler Resource Constraint", myDetails, myDominoServer.Location)
				End If
			End If
		...
		Finally
			'Update the Traveler_Status table with one of the queries above
			objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
		End Try
		...
		Try
			...
			strSQL = "Update Traveler_Status SET Devices='" & myDominoServer.Traveler_DeviceCount & "' WHERE ServerName='" & myDominoServer.Name & "'"
			WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
			objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
		...
		Try
			...
			If myDominoServer.Traveler_UserCount <> -1 Then
				strSQL = "Update Traveler_Status SET Users='" & myDominoServer.Traveler_UserCount & "' WHERE ServerName='" & myDominoServer.Name & "'"
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
			End If
		...
		Try
			If myDominoServer.Traveler_Status <> "" Then
				strSQL = "Update Traveler_Status SET  Status='" & myDominoServer.Traveler_Status & "' WHERE ServerName='" & myDominoServer.Name & "'"
				WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
			End If
		...
		Try
			If myDominoServer.Traveler_Details <> "" Then
				strSQL = "Update Traveler_Status SET  Details='" & myDominoServer.Traveler_Details & "' WHERE ServerName='" & myDominoServer.Name & "'"
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
			End If
		...
		Try
			If myDominoServer.HTTP_Configured_Max_Sessions <> 0 Then
				strSQL = "Update Traveler_Status SET HTTP_MaxConfiguredConnections='" & myDominoServer.HTTP_Configured_Max_Sessions & "' WHERE ServerName='" & myDominoServer.Name & "'"
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
		...
		Try
			If myDominoServer.HTTP_Actual_Max_Sessions <> 0 Then
				strSQL = "Update Traveler_Status SET HTTP_PeakConnections='" & myDominoServer.HTTP_Actual_Max_Sessions & "' WHERE ServerName='" & myDominoServer.Name & "'"
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
				'  If MyLogLevel = LogLevel.Verbose Then WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " This server is configured to serve " & myDominoServer.HTTP_Configured_Max_Sessions & " simultaneous HTTP sessions.")
			End If
		...
		Try
			With myDominoServer
				If .HTTP_Actual_Max_Sessions * 1.2 > .HTTP_Configured_Max_Sessions And .HTTP_Actual_Max_Sessions > 1 And .HTTP_Configured_Max_Sessions <> 0 Then
                    ...
                    Details = "Warning"
					myAlert.QueueAlert("Domino", .Name, "Traveler Threads Warning", Details, .Location, "Traveler")
				Else
					myAlert.ResetAlert("Domino", .Name, "Traveler Threads Warning", .Location, "The Traveler server has sufficient HTTP threads", "Traveler")
				End If
				...
				If .HTTP_Actual_Max_Sessions = .HTTP_Configured_Max_Sessions And .HTTP_Actual_Max_Sessions > 1 And .HTTP_Configured_Max_Sessions <> 0 Then
                    ...
                    myAlert.QueueAlert("Domino", .Name, "Traveler Insufficient Threads", Details, .Location, "Traveler")
				Else
					myAlert.ResetAlert("Domino", .Name, "Traveler Insufficient Threads", .Location)
				End If
			End With
		Catch ex As Exception
		...
		Try
			...
			If myActiveUsers = 0 Then
				strSQL = "Update Status SET Description='No Traveler Users are Active' WHERE Name='" & myDominoServer.Name & "'"
				' WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
			End If
		Catch ex As Exception
		...
		Try
			...
			If boolHTTPRunning = False Then
                ...
                strSQL = "Update Traveler_Status SET Status='Failure', HTTP_Status='" & Status & "', HTTP_Details='n/a' WHERE ServerName='" & myDominoServer.Name & "'"
				WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
				myDominoServer.Description = "Traveler is disabled because HTTP is not running"

				strSQL = "Update Status SET Description='Traveler is disabled because HTTP is not running' WHERE Name='" & myDominoServer.Name & "'"
				' WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
			End If
		Catch ex As Exception
		...
		Try
			...
			If boolTravelerRunning = False Then
				...
				strSQL = "Update Traveler_Status SET Status='Failure',  Details='" & Details & "' WHERE ServerName='" & myDominoServer.Name & "'"
				WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
				myDominoServer.Description = "Traveler is not running"

				strSQL = "Update Status SET Description='Traveler is not running' WHERE Name='" & myDominoServer.Name & "'"
				' WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
				objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
			Else
		...
		With myDominoServer
			...
			Try
				If InStr(.Statistics_Traveler, "Traveler.DeviceSync.Count.200") > 0 Then
					...
					strSQL = "Update Traveler_Status SET  IncrementalSyncs='" & intSuccessfulSyncCount & "' WHERE ServerName='" & myDominoServer.Name & "'"
					' WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
					objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)

					UpdateDominoDailyStatTable(myDominoServer.Name, "Traveler.IncrementalDeviceSyncs", intSuccessfulSyncCount)
					...
					If .Traveler_Successful_DeviceSync_Count = .Traveler_Previous_Successful_DeviceSync_Count Then
						...
						Try
							...
							If ScanInterval.TotalMinutes > 30 Then
								...
								myAlert.QueueAlert("Domino", .Name, "Traveler Device Sync", "The device synchronization counts for the Lotus Traveler server task haven't changed in " & ScanInterval.TotalMinutes & " minutes, and this COULD indicate a problem with Lotus Traveler. Then again, it could just indicate that nobody has been retrieving their mail remotely in a while.  Still, VitalSigns thought you should know.  When the number changes you'll get another message stating that fact.", .Location)
								strSQL = "Update Traveler_Status SET  Status='" & "Not Syncing" & "' WHERE ServerName='" & myDominoServer.Name & "'"
								WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
								objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
							End If
						Catch ex As Exception
					...
					Else
						myAlert.ResetAlert("Domino", .Name, "Traveler Device Sync", .Location)
					End If
				Else

					UpdateDominoDailyStatTable(myDominoServer.Name, "Traveler.IncrementalDeviceSyncs", 0)
			...
			Try
				If InStr(myDominoServer.Statistics_Traveler, "Traveler.Availability.Index.Current") Then
					...
					strSQL = "Update Traveler_Status SET  Availability_Index='" & myDominoServer.Traveler_Availability_Index & "' WHERE ServerName='" & myDominoServer.Name & "'"
					WriteDeviceHistoryEntry("Domino", myDominoServer.Name, Now.ToString & " " & strSQL)
					objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
            ...
            Try
                If InStr(myDominoServer.Statistics_Traveler, "Traveler.Memory.Java.Current") > 0 Then
                    ...
                    UpdateDominoDailyStatTable(myDominoServer.Name, "Traveler.Memory.Java.Current", myDominoServer.Traveler_Memory_Java)
                End If
            ...
            Try
                If InStr(myDominoServer.Statistics_Traveler, "Traveler.Memory.C.Current") > 0 Then
                    ...
                    UpdateDominoDailyStatTable(myDominoServer.Name, "Traveler.Memory.C.Current", myDominoServer.Traveler_Memory_C)
                End If
	...
	Private Sub CheckTravelerState(ByRef myDominoServer As MonitoredItems.DominoServer)
		...
		Try
            If myDominoServer.Traveler_Status = "Red" Then
                ...
                '5/20/2016 NS modified for VSPLUS-2874
                strSQL = "Update Status SET  Status='Traveler Status: Red', StatusCode='Issue', Details = 'The overall status of IBM Notes Traveler is Red' WHERE TypeANDName ='" & myDominoServer.Name & "-Domino'"
                objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
            End If
		...
		Try
            If myDominoServer.Traveler_Status = "Yellow" Then
                ...
                myDominoServer.StatusCode = "Issue"
                '5/20/2016 NS modified for VSPLUS-2874
                strSQL = "Update Status SET  Status='Traveler Status: Yellow', StatusCode='Issue', Details = 'The overall status of IBM Notes Traveler is Yellow' WHERE TypeANDName ='" & myDominoServer.Name & "-Domino'"
                objVSAdaptor.ExecuteNonQueryAny("VitalSigns", "Status", strSQL)
            End If
		...
		Try
			Do Until boolTimeToStop = True
				...
				If InMaintenance("Domino", MyDominoServer.Name) = False Then
					Try
						CheckTravelerUsersUsingJSON(MyDominoServer)
					Catch ex As Exception
	...
	Private Sub getTravelerDeviceMoreDetailsWrapper()
		Do Until boolTimeToStop
			...
			Dim sSQLServers As String = "SELECT DISTINCT ServerName FROM TRAVELER_DEVICES_TEMP WHERE (IsMoreDetailsFetched IS NULL OR IsMoreDetailsFetched=0) AND ISNULL(MoreDetailsURL,'') != '' AND ISNULL(ServerName,'') !=''"
			Dim dtServers As DataTable = myAdapter.FetchData(myConnectionString.GetDBConnectionString("VitalSigns"), sSQLServers)
			WriteDeviceHistoryEntry("All", "Traveler_Users_MoreDetails", Now.ToString & " Distinct Server List Count: " & dtServers.Rows.Count & " Servers ", LogLevel.Verbose)
			For Each drServers As DataRow In dtServers.Rows
	...
	Private Sub getTravelerDeviceMoreDetails(ByVal data As Object)
		Dim parameters As New MyParameters
		parameters = CType(data, MyParameters)
		Dim moreInfoSQLs As List(Of System.Data.SqlClient.SqlCommand)
		Dim dr As DataRow
		dr = parameters.dr
		moreInfoSQLs = parameters.moreInfoSQL
		...
		Dim href As String = dr("MoreDetailsURL").ToString()
		...
		Dim ServerName As String = dr("ServerName").ToString()
		...
		If myMasterDevices IsNot Nothing Then
			If myMasterDevices.data IsNot Nothing Then
				...
				For Each myTravelerDevice As TravelerDevice In myMasterDevices.data
					...
					Try
						addTravelerDeviceMoreInfoToCollection(moreInfoSQLs, myTravelerDevice)
					Catch ex As Exception
...
Alerts:
		...
		Try
            If DominoServer.Traveler_Status = "Yellow" Then
                '5/20/2016 NS modified for VSPLUS-2874
                myAlert.QueueAlert("Domino", DominoServer.Name, "Traveler Status: Yellow", "The overall status of IBM Notes Traveler on " & DominoServer.Name & " is " & DominoServer.Traveler_Status & " due to: " & vbCrLf & strReasons, DominoServer.Location)
            Else
                Dim strDetails As String
                If DominoServer.Traveler_Status = "Green" Then
                    strDetails = "The overall status of IBM Notes Traveler is Green"
                    '5/20/2016 NS modified for VSPLUS-2874
                    myAlert.ResetAlert("Domino", DominoServer.Name, "Traveler Status: Yellow", DominoServer.Location, strDetails, "Traveler")
                End If
    ...
    Private Sub CheckTravelerServlet(ByRef MyDominoServer As MonitoredItems.DominoServer)
		...
		Try
            If InStr(myServletURL.HTML, "Traveler server is available") Or InStr(myServletURL.HTML, "Traveler está disponível") Or InStr(myServletURL.HTML, "ist verfügbar") Or InStr(myServletURL.HTML, "är tillgänglig") Then
                ...
                myAlert.ResetAlert("Domino", MyDominoServer.Name, "Traveler Servlet", MyDominoServer.Location)
            ...
            Else
                ...
                myAlert.QueueAlert("Domino", MyDominoServer.Name, "Traveler Servlet", "The Traveler Servlet is not responding.", MyDominoServer.Location)
