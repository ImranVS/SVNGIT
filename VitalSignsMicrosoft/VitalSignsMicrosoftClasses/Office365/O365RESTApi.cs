...
namespace VitalSignsMicrosoftClasses
{
	public class O365RESTApi
	{
					...
					if (myDevices != null)
					{
						if (myDevices.NameSpaceType == "Federated")
						{
							...
							if (response =="-1")
							{
								...
								Common.makeAlert(false , myServer, commonEnums.AlertType.ADFS, ref AllTestsList, "ADFS Redirect Failed", "Connectivity");
							}
							else
							{
			...
			try
			{
				...
				if (response != "")
				{
					...
					if (response == "-1")
					{
						...
						Common.makeAlert(false, myServer, commonEnums.AlertType.URL, ref AllTestsList, "O365 URL connection Failed", "Connectivity");
					}
					else
					{
						myServer.URLTest = true;
						Common.makeAlert(true, myServer, commonEnums.AlertType.URL, ref AllTestsList, "Successfully connected to the O365 URL", "Connectivity");
		...
		//private void doStaleEmailTest(MonitoredItems.Office365Server myServer, ref TestResults AllTestsList)
		//{
		//    string fromDate = DateTime.Now.Year.ToString() + "-" + DateTime.Now.Month.ToString() + "-" + DateTime.Now.Day.ToString() + "T" + "00" + ":00:00";
		//    string URL = "https://reports.office365.com/ecp/reportingwebservice/reporting.svc/StaleMailboxDetail?$select=DaysInactive,UserName&$filter=Date%20ge%20datetime%27" + fromDate + "%27&$top=100&$format=JSON";
		//    string retVal = getODATA(URL, myServer.UserName, myServer.Password);
		//    if (retVal != "")
		//    {
		//        StaleMailBox myDevices = new StaleMailBox();
		//        DataContractJsonSerializer serializer = new DataContractJsonSerializer(myDevices.GetType());
		//        MemoryStream ms = new MemoryStream(System.Text.Encoding.Unicode.GetBytes(retVal));
		//        try
		//        {
		//            myDevices = (StaleMailBox)serializer.ReadObject(ms);
		//            foreach (StaleMailBoxData smd in myDevices.d)
		//            {
		//                int daysInactive = smd.DaysInactive;
		//                string userName = smd.UserName;
		//            }
		...
		private string getODATA(string URL , string userId,string pwd)
		{
			string returnVal = "";
			try
			{
				//string url = "https://reports.office365.com/ecp/reportingwebservice/reporting.svc/StaleMailboxDetail?$select=DaysInactive,UserName&$filter=Date%20ge%20datetime%272016-02-02T00:00:00%27&$top=100&$format=JSON";
				HttpWebRequest req = HttpWebRequest.Create(URL) as HttpWebRequest;

				req.Method = "GET";
				req.Headers.Add("DNT", "1");
				string auth = "Basic " + Convert.ToBase64String(System.Text.Encoding.Default.GetBytes(userId + ":" + pwd));
				req.PreAuthenticate = true;
				req.Headers.Add("Authorization", auth);
				//req.Headers.Add("Host", "reports.office365.com");
				req.Host = "reports.office365.com";
		...
		private static Microsoft.SharePoint.Client.ListItem GetDocumentFromSP(string documentName, System.Net.ICredentials creds, string URL, string documentList)
		{
...
//        }
	...
	public class StaleMailBoxMetadata
	{
	...
	public class StaleMailBoxData
	{
		public StaleMailBoxMetadata __metadata { get; set; }
		public string UserName { get; set; }
		public int DaysInactive { get; set; }
	}

	public class StaleMailBox
	{
		public List<StaleMailBoxData> d { get; set; }
	}
	class Product
	{
