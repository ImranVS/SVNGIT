...
//Soma2494
				try
				{
					ScanASAP = myRegistry.ReadFromRegistry("ScanADASAP").ToString();
				}
				catch (Exception ex)
				{
					ScanASAP = "";
					...
					try
					{
						if (thisServer != null && !thisServer.IsBeingScanned)
						{
							...
							else
							{
								...
								DateTime thTime = DateTime.Now.AddMinutes(5);
				...
				foreach (DataRow row in dt.Rows)
				{
					...
					switch (row["Tests"].ToString())
					{
						...
						case "Inbox Test":
							...
							break;
						case "Compose Email":
							MyO365Server.EnableComposeEmailTest = Convert.ToBoolean(row["EnableSimulationTests"].ToString());
						...
						case "OneDrive Upload Document":
							...
							break;
						case "OneDrive Download Document":
		...
		protected void InitStatusTable(MonitoredItems.Office365ServersCollection collection)
		{
			try
			{
				...
				if (type != "")
				{
					String SqlStr = "DELETE FROM Status WHERE Type='" + type + "' AND CATEGORY='" + NodeName +"'";
					db.Execute(SqlStr);
					...
					foreach (MonitoredItems.Office365Server server in collection)
					{
						...
						if (server.Location != "")
						{
							...
							SqlStr += " ('" + type + "', '" + server.Location + "', '" + server.Category + "', '" + server.Name + "', '" + server.Status + "', 'This server has not yet been scanned.', 'Microsoft " + type + " Server', '" +
								server.Name + "-" + server.Location + "', '" + server.StatusCode + "'),";
						}
					}
					if (i > 0)
					{
