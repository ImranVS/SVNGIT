...
#Region "Create Collections of items to Monitor"
    ...
    Private Sub CreateDominoServersCollection()
        ...
        Try
            ...
            strSQL = "SELECT ServerName AS Name, [Scan Interval], Category, Enabled, PendingThreshold, DeadThreshold, Location, Loc.ID as LocationID, ServerType, MailDirectory, OffHoursScanInterval, RetryInterval,   ds.ServerID AS [Key], ResponseThreshold, srv.IPAddress, " & _
            "BES_Server, BES_Threshold, FailureThreshold, SearchString, DeadMailDeleteThreshold, DiskSpaceThreshold, HeldThreshold, ScanDBHealth, Description, NotificationGroup, Memory_Threshold, CPU_Threshold, Cluster_Rep_Delays_Threshold, " & _
            "EXJStartTime,EXJDuration,EXJLookBackDuration,EXJEnabled " & _
            " FROM dbo.DominoServers ds " & _
            " inner join dbo.Servers srv on ds.ServerID = srv.ID " & _
            " inner join dbo.ServerTypes srvt on srv.ServerTypeId=srvt.ID " & _
            " inner join dbo.Locations Loc on loc.ID=srv.LocationID "
			...
			If System.Configuration.ConfigurationManager.AppSettings("VSNodeName") <> Nothing Then
				...
				strSQL += "inner join DeviceInventory di on srv.ID=di.DeviceID and di.DeviceTypeId=srv.ServerTypeId " & _
				 "inner join Nodes on Nodes.ID=di.CurrentNodeId and Nodes.Name='" & nodeName & "' "
        ...
        Try
            ...
            If MyDominoServers.Count > 0 Then
                ...
                For Each myDataRow In dsDominoServers.Tables("DominoServers").Rows()
                    WriteAuditEntry(Now.ToString & " I have " & myDataRow.Item("Name"))
        ...
        Try
            ...
            For Each dr In dsDominoServers.Tables("DominoServers").Rows
                ...
                With MyDominoServer
                    If MyLogLevel = LogLevel.Verbose Then WriteAuditEntry(Now.ToString & " Configuring Domino Server: " & dr.Item("Name"))
                    ...
                    Try
                        If dr.Item("EXJEnabled") Is Nothing Then
                            .EXJEnabled = False
                        Else
                            .EXJEnabled = dr.Item("EXJEnabled")
                        End If
                    Catch ex As Exception
                        .Location = False
                    End Try
                    ...
                    Try
                        If dr.Item("EXJStartTime") Is Nothing Then
                            .EXJStartTime = ""
                        Else
                            .EXJStartTime = dr.Item("EXJStartTime")
                        End If
                    Catch ex As Exception
                        .EXJStartTime = ""
                    End Try
                    ...
                    Try
                        If dr.Item("EXJDuration") Is Nothing Then
                            .EXJDuration = 0
                        Else
                            .EXJDuration = dr.Item("EXJDuration")
                        End If
                    Catch ex As Exception
                        .EXJDuration = 0
                    End Try
                    ...
                    Try
                        If dr.Item("EXJLookBackDuration") Is Nothing Then
                            .EXJLookBackDuration = 0
                        Else
                            .EXJLookBackDuration = dr.Item("EXJLookBackDuration")
                        End If
                    Catch ex As Exception
                        .EXJLookBackDuration = 0
                    End Try
