...
{
	public partial class ActiveDirectoryServerDetailsPage3 : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!IsPostBack && !IsCallback)
			{
				...
				if (Session["UserPreferences"] != null)
				{
					...
					{
                        //13/05/2016 Durga Modified for VSPLUS-2956 

                        if (dr[1].ToString() == "ActiveDirectoryServerDetailsPage3|MonitoredServicesGrid")
						{
							MonitoredServicesGrid.SettingsPager.PageSize = Convert.ToInt32(dr[2]);
						}
                        if (dr[1].ToString() == "ActiveDirectoryServerDetailsPage3|EventsHistory")
                        {
                            EventsHistory.SettingsPager.PageSize = Convert.ToInt32(dr[2]);
                        }
                        if (dr[1].ToString() == "ActiveDirectoryServerDetailsPage3|UnMonitoredServicesGrid")
                        {
                            UnMonitoredServicesGrid.SettingsPager.PageSize = Convert.ToInt32(dr[2]);
                        }
						if (dr[1].ToString() == "ActiveDirectoryServerDetailsPage3|maintenancegrid")
						{
							maintenancegrid.SettingsPager.PageSize = Convert.ToInt32(dr[2]);
						}
                        if (dr[1].ToString() == "ActiveDirectoryServerDetailsPage3|AlertGridView")
						{
							AlertGridView.SettingsPager.PageSize = Convert.ToInt32(dr[2]);
						}
                        if (dr[1].ToString() == "ActiveDirectoryServerDetailsPage3|OutageGridView")
						{
			...
			else
			{
				...
				FillReplicationGridViewFromSession();
				//Web Admin tab
			}
			//10/7/2014 NS added for VSPLUS-934
			SetGraph("hh", servernamelbl.Text);
		...
		protected void EventsHistoryGridView_PageSizeChanged(object sender, EventArgs e)
		{
            ...
            VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("ActiveDirectoryServerDetailsPage3|EventsHistory", EventsHistory.SettingsPager.PageSize.ToString(), Convert.ToInt32(Session["UserID"]));
		...
		protected void EditInConfigButton_Click(object sender, System.EventArgs e)
		{
			...
			if (Request.QueryString["Name"] != "" && Request.QueryString["Name"] != null)
			{
				...
				Response.Redirect("~/Configurator/ActiveDirectoryProperties.aspx?ID=" + id + "&Name=" + name + "&Cat=" + dt.Rows[0]["ServerType"].ToString() + "&Loc=" + dt.Rows[0]["Location"].ToString() + "&ipaddr=" + dt.Rows[0]["ipaddress"].ToString(), false);//Mukund, 05Aug14, VSPLUS-844:Page redirect on callback
		...
		public DataTable SetGraph(string paramGraph, string DeviceName)
		{
			try
			{
				...
				if (dt.Rows.Count > 0)
				{
					...
					DataTable dt1 = VSWebBL.DashboardBL.ActiveDirectoryServerDetailsBL.Ins.ResponseThreshold(DeviceName);
					if (dt1.Rows.Count > 0)
					{
					...
					seriesXY.AxisY.Title.Text = "AD Query Responsetime";
		...
		{
			//ProfilesGridView.PageIndex;
            VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("ActiveDirectoryServerDetailsPage3|MonitoredServicesGrid", MonitoredServicesGrid.SettingsPager.PageSize.ToString(), Convert.ToInt32(Session["UserID"]));
			Session["UserPreferences"] = VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.GetUserRowPrefrenceDetails(Convert.ToInt32(Session["UserID"]));
		}
        protected void UnMonitoredServicesGrid_PageSizeChanged(object sender, EventArgs e)
        {
            //ProfilesGridView.PageIndex;
            VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("ActiveDirectoryServerDetailsPage3|UnMonitoredServicesGrid", UnMonitoredServicesGrid.SettingsPager.PageSize.ToString(), Convert.ToInt32(Session["UserID"]));
            Session["UserPreferences"] = VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.GetUserRowPrefrenceDetails(Convert.ToInt32(Session["UserID"]));
        }
		...
		{
			DataTable dt = VSWebBL.DashboardBL.ActiveDirectoryServerDetailsBL.Ins.SetGraphForDiskSpace(serverName);
		...
		public void FillReplicationGridViewFromSession()
		{
			try
			{
				if (Session["ReplicationGridViewSession"] != null)
				{
					DataTable dt = Session["ReplicationGridViewSession"] as DataTable;
					ReplicationGridView.DataSource = dt;
		...
		private void FillReplicationGridView()
		{
			try
			{

				DataTable StatusTable12 = VSWebBL.DashboardBL.ActiveDirectoryServerDetailsBL.Ins.GetReplicationSumamryData(servernamelbl.Text);

				Session["ReplicationGridViewSession"] = StatusTable12;
				ReplicationGridView.DataSource = StatusTable12;
				ReplicationGridView.DataBind();
			}
			catch (Exception ex)
			{
