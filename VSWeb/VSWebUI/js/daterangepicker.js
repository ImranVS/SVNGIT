...
!function ($) {
	...
	DateRangePicker.prototype = {
		...
		buildCalendar: function (month, year) {
			...
			//alert( Date.create('today').set({ day: 1, month: month, year: year }));
			var lastMonth = firstDay.clone().addDays(-1).getMonth();
			//alert(firstDay.getMonth());
			var lastYear = firstDay.clone().addDays(-1).getFullYear();
			//alert(lastYear);
			//Sowjanya 1628 ticket
			var daysInMonth = this.getDaysInMonth(year, month+1);
			//alert(daysInMonth);
			var daysInLastMonth = this.getDaysInMonth(lastYear, lastMonth+1);
			
			//alert(daysInLastMonth);
