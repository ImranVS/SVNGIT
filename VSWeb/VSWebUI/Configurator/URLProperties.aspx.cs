...
{
    public partial class URLProperties : System.Web.UI.Page
    {
        string Mode;
        ...
        protected void Page_Load(object sender, EventArgs e)
		{
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["UserPreferences"] != null)
                {
                    ...
                    {
                        if (dr[1].ToString() == "URLProperties|MaintWinListGridView")
                        {
                            MaintWinListGridView.SettingsPager.PageSize = Convert.ToInt32(dr[2]);
                        }
                        if (dr[1].ToString() == "URLProperties|AlertGridView")
                        {
            ...
            try
            {
                ...
                if ((Request.QueryString["ID"] != null && Request.QueryString["ID"] != "")||(Request.QueryString["Key"] != null && Request.QueryString["Key"] != ""))
                {
                    ...
                    if (!IsPostBack)
                    {
                        ...
                        //URLRoundPanel.HeaderText = "URL Properties -  "+ " "  + NameTextBox.Text;
        ...
        private void FillData(string ID)
        {
            try
            {
                ...
                URLsObject.ID = ID;
                ReturnDCObject = VSWebBL.ConfiguratorBL.URLsBL.Ins.GetData(URLsObject);
                ...
                IPAddressTextBox.Text = ReturnDCObject.TheURL;
                RequiredTextBox.Text = ReturnDCObject.SearchStringNotFound;
                //txtSearch.Text = ReturnDCObject.SearchStringFound;
				if (ReturnDCObject.SearchStringFound == "0" || ReturnDCObject.SearchStringFound == "")
					AlertConditionRB.SelectedIndex = 0;
				else
					AlertConditionRB.SelectedIndex = 1;
                ...
                LocationComboBox.Text = ReturnDCObject.Location;
                ...
                Session["ReturnUrl"] = "URLProperties.aspx?ID=" + ID + "&tab=1";
        ...
        private void UpdateURLsData()
        {
            try
            {


                URLs UrlObj = new URLs();
                //11/19/2013 NS added ID below
                UrlObj.ID = ID;
                UrlObj.TheURL = IPAddressTextBox.Text;
                UrlObj.Name = NameTextBox.Text;
                DataTable returntable = VSWebBL.ConfiguratorBL.URLsBL.Ins.GetIPAddress(UrlObj,"Update");

                if (returntable.Rows.Count > 0 && (!string.IsNullOrEmpty(IPAddressTextBox.Text)) )
                {
                    ...
                    errorDiv.InnerHtml = "This URL Address/Name is already being monitored. Please enter another IP Address/Name."+
                ...
                else
                {
                    try
                    {
                        Object ReturnValue = VSWebBL.ConfiguratorBL.URLsBL.Ins.UpdateData(CollectDataForURLs());
                        ...
                        if (ReturnValue.ToString() == "True")
                        {
                            ...
                            ErrorMessagePopupControl.ShowOnPageLoad = true;
                            */
                            Session["URLUpdateStatus"] = NameTextBox.Text;
        ...
        private void InsertURLs()
        {
			try
			{
				//DataTable dt = VSWebBL.ConfiguratorBL.URLsBL.Ins.GetAllData();
				//if (dt.Rows.Count > 0)
				//{
					URLs UrlObj = new URLs();
					UrlObj.TheURL = IPAddressTextBox.Text;
					UrlObj.Name = NameTextBox.Text;
					DataTable returntable = VSWebBL.ConfiguratorBL.URLsBL.Ins.GetIPAddress(UrlObj, "Insert");
				...
				else
				{
					...
					try
					{
						object ReturnValue = VSWebBL.ConfiguratorBL.URLsBL.Ins.InsertData(CollectDataForURLs());
						SetFocusOnError(ReturnValue);
						if (ReturnValue.ToString() == "True")
						{
							//3/19/2014 NS modified
        ...
        protected void FormOkButton_Click(object sender, EventArgs e)
        {
            try
            {
                VSWebBL.SettingBL.SettingsBL.Ins.UpdateSvalue("URL Update", "True", VSWeb.Constants.Constants.SysString);
            }
            catch (Exception ex)
            {
                //6/27/2014 NS added for VSPLUS-634
        ...
        private Status CollectDataforStatus()
        {
            ...
            try
            {
               ...
               // St.PendingMail = 0;
                ...
                St.ResponseTime = 0;
                St.TypeANDName =NameTextBox.Text + "-URL";
                St.Icon = 0;
        ...
        protected void CategoryTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            string myName = CategoryTextBox.Text;
            try
            {
                if (myName.IndexOf("'") > 0)
                    myName = myName.Replace("'", "");
                ...
                if (myName !=CategoryTextBox.Text)
                    CategoryTextBox.Text = myName;
            ...
            private void FillAlertGridView()
        {
            try
            {
                ...
                AlertDataTable = VSWebBL.ConfiguratorBL.AlertsBL.Ins.GetAlertTab(NameTextBox.Text,"URL");
        ...
        protected void MaintWinListGridView_PageSizeChanged(object sender, EventArgs e)
        {
            ...
            VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("URLProperties|MaintWinListGridView", MaintWinListGridView.SettingsPager.PageSize.ToString(), Convert.ToInt32(Session["UserID"]));
        ...
        {
            //ProfilesGridView.PageIndex;
            VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("URLProperties|AlertGridView", AlertGridView.SettingsPager.PageSize.ToString(), Convert.ToInt32(Session["UserID"]));
            Session["UserPreferences"] = VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.GetUserRowPrefrenceDetails(Convert.ToInt32(Session["UserID"]));
        }
		...
		protected void ASPxMenu1_ItemClick(object source, DevExpress.Web.MenuItemEventArgs e)
		{
			...
			if (Request.QueryString["ID"] != "" && Request.QueryString["ID"] != null)
			{
				...
				dt = (VSWebBL.SecurityBL.ServersBL.Ins.GetURLDetailsByID(Convert.ToInt32(Request.QueryString["ID"])));
				if (dt.Rows.Count > 0)
				{
					Response.Redirect("~/Dashboard/URLDetails.aspx?Name=" + dt.Rows[0]["Name"].ToString() + "&Type=" + dt.Rows[0]["Type"].ToString() + "&LastDate=" + dt.Rows[0]["LastUpdate"].ToString(), false);//Mukund, 05Aug14, VSPLUS-844:Page redirect on callback
					Context.ApplicationInstance.CompleteRequest();
				}
			}
