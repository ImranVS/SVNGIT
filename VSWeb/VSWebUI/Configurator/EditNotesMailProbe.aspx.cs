...
{
    public partial class EditNotesMailProbe : System.Web.UI.Page
    {
        string NotesName;
        string MailID;
        string Mode;
       ...
       // string UploadDirectory ;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["UserPreferences"] != null)
                {
                    ...
                    {
                        if (dr[1].ToString() == "EditNotesMailProbe|MaintWinListGridView")
                        {
                            MaintWinListGridView.SettingsPager.PageSize = Convert.ToInt32(dr[2]);
                        }
                        if (dr[1].ToString() == "EditNotesMailProbe|AlertGridView")
                        {
            ...
            try
            {
                ...
                {
                    Mode = "Update";
					gearmenu.Visible = true;//VSPLUS:2153 change the pin wheel visible true--somaraj
                    //MailID = Request.QueryString["NotesMailAddress"];
                    NotesName = Request.QueryString["Name"];
                    ...
                    if (!IsPostBack)
                    {
                        ...
                        //NotesMailProbeRoundPanel.HeaderText = "Notes Mail Probe -" +"  "+ NameTextBox.Text;
                ...
                else
                {
                    ...
                    if (!IsPostBack)
                    {
                        ...
                        NameTextBox.Focus();
						gearmenu.Visible = false;//VSPLUS-2153 task put pin wheel visible false--somaraj
                    }
                }
        ...
        protected void formOKButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                VSWebBL.SettingBL.SettingsBL.Ins.UpdateSvalue("NotesMail Probe Update", "True", VSWeb.Constants.Constants.SysString);
            }
            catch (Exception ex)
            {
                //6/27/2014 NS added for VSPLUS-634
            ...
            try
            {
                if (Mode == "Update")
                {
                    ...
                    UpdateNotesMailProbeData();
                }
                if (Mode == "Insert")
                {
                   
                    InsertNotesMailProbe();
                    if (flag == false)
                        InsertStatus();
                    else
                    {
        ...
        private void FillTargetServerComboBox()
        {
            DataTable DSTasksDataTable = RestrServers();
            TargetComboBox.DataSource = DSTasksDataTable;
            TargetComboBox.TextField = "ServerName";
            TargetComboBox.ValueField = "ID";
            TargetComboBox.DataBind();
        }

        public DataTable RestrServers()
        {
			DataTable DCTasksDataTable = VSWebBL.ConfiguratorBL.NotesMailProbeBL.Ins.GetDominoServerNames();
        ...
        protected void EchoServiceCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (EchoServiceCheckBox.Checked == true)
            {
                ReplyToTextBox.Enabled = true;

            }
            else
            {
               ReplyToTextBox.Enabled = false;
            }
        }
        ...
        private Status CollectDataforStatus()
        {
            ...
            try
            {
                ...
                //St.DeadMail = 0;
                St.Description = "Sends NotesMail to target address and verifies delivery.";
                // St.Details = "";
                St.DownCount = 0;
                //St.Location = "Cluster";
               ...
               // St.PendingMail = 0;
                ...
                St.Type = "NotesMail Probe";
                St.Upcount = 0;
                St.UpPercent = 1;
                St.LastUpdate = System.DateTime.Now;
                St.ResponseTime = 0;
                //5/6/2016 NS modified
                //Changed -NMP to -NotesMail Probe
                St.TypeANDName = NameTextBox.Text + "-NotesMail Probe";
                St.Icon = 0;
        ...
        private void FillMaintenanceGrid()
        {
            try
            {
                ...
                MaintDataTable = VSWebBL.ConfiguratorBL.MaintenanceBL.Ins.GetMaintenDataOnServerIDs(SourceServerComboBox.Text,TargetComboBox.Text,"");
        ...
        protected void AlertGridView_PageSizeChanged(object sender, EventArgs e)
        {
            ...
            VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("EditNotesMailProbe|AlertGridView", AlertGridView.SettingsPager.PageSize.ToString(), Convert.ToInt32(Session["UserID"]));
            Session["UserPreferences"] = VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.GetUserRowPrefrenceDetails(Convert.ToInt32(Session["UserID"]));
        }
        protected void MaintWinListGridView_PageSizeChanged(object sender, EventArgs e)
        {
            //ProfilesGridView.PageIndex;
            VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.UpdateUserPreferences("EditNotesMailProbe|MaintWinListGridView", MaintWinListGridView.SettingsPager.PageSize.ToString(), Convert.ToInt32(Session["UserID"]));
            Session["UserPreferences"] = VSWebBL.ConfiguratorBL.UserPreferencesBL.Ins.GetUserRowPrefrenceDetails(Convert.ToInt32(Session["UserID"]));
        }
		protected void ASPxMenu1_ItemClick(object source, DevExpress.Web.MenuItemEventArgs e)
		{
			...
			if (Request.QueryString["Name"] != "" && Request.QueryString["Name"] != null)
			{
				dt = (VSWebBL.SecurityBL.ServersBL.Ins.GetServerDetailsByName_Mail(Request.QueryString["Name"].ToString()));
				if (dt.Rows.Count > 0)
				{
                    //9/14/2015 NS modified for VSPLUS-2148
					//Response.Redirect("~/Dashboard/DominoServerDetailsPage2.aspx?Name=" + dt.Rows[0]["Name"].ToString() + "&Type=" + dt.Rows[0]["Type"].ToString() + "&Status=" + dt.Rows[0]["Status"].ToString() + "&LastDate=" + dt.Rows[0]["LastUpdate"].ToString(), false);//Mukund, 05Aug14, VSPLUS-844:Page redirect on callback
                    Response.Redirect("~/Dashboard/NotesMailProbeDetailsPage.aspx?Name=" + dt.Rows[0]["Name"].ToString() + "&Type=" + dt.Rows[0]["Type"].ToString() + "&Status=" + dt.Rows[0]["Status"].ToString() + "&LastDate=" + dt.Rows[0]["LastUpdate"].ToString(), false);//Mukund, 05Aug14, VSPLUS-844:Page redirect on callback
					Context.ApplicationInstance.CompleteRequest();
				}
			}
