...
{
	public class LatencyTestDAL
	{
		private AdaptorforDsahBoard objAdaptor = new AdaptorforDsahBoard();
		private Adaptor adaptor = new Adaptor();
		private static LatencyTestDAL _self = new LatencyTestDAL();

		public static LatencyTestDAL Ins
		{
			get
			{
				return _self;
		...
		public DataTable getNetworkLatencyHeatMap(string NetworkLatencyID)
		{
			...
			try
			{
				dt = objAdaptor.GetDataFromProcedure("GetNetworkLatencyHeatmap", "NetworkLatencyID", NetworkLatencyID);
			}
			catch
			{
			}
		...
		public DataTable getNetworkLatencyTestNames()
		{
			...
			try
			{
				dt = adaptor.FetchData("select TestName, NetworkLatencyId from NetworkLatency where Enable = 1");
			}
			catch
			{
			}
