...
#Region "Threads"
	...
	Protected Sub DailyTasks()
		...
		Do 'While TimeToStop = False
			...
			Else
				...
				'VSPLUS-416,19Dec14
				Dim strSQL1 As String = "select * from (select ServerName from dbo.DominoServers ds, dbo.Servers srv, dbo.ServerTypes srvt, dbo.Locations Loc where ds.ServerID = srv.ID And srv.ServerTypeID = srvt.id And srv.LocationID = Loc.ID and Enabled=1 except select Name from (select ServerName AS Name,(Select MAX(ScanDate) From VSS_Statistics.dbo.Daily where server=srv.servername) AS 'MyDate' from dbo.DominoServers ds, dbo.Servers srv, dbo.ServerTypes srvt, dbo.Locations Loc where ds.ServerID = srv.ID And srv.ServerTypeID = srvt.id And srv.LocationID = Loc.ID and Enabled=1) a  where DATEDIFF(hh ,mydate,getdate())<=24) as totalrows"
				'WriteAuditEntry(Now.ToString & ":Mukund:" & strSQL1)
				Dim dt As New DataTable
				Dim ds As New DataSet
				dt.TableName = "DominoServers"
				ds.Tables.Add(dt)

				vsobj.FillDatasetAny("VitalSigns", "vitalsigns", strSQL1, ds, "DominoServers")
				'WriteAuditEntry(Now.ToString & ":Mukund:" & dt.Rows.Count.ToString)


				If LastDate.Date <> FixDate(Now) Or dt.Rows.Count > 0 Then
				...
				If boolDominoService Then
					...
					Try
						...
						'intTotalRowCount = Convert.ToDouble(vsobj.ExecuteScalarAny("VitalSigns", "Status", strSQL))
						Dim dt1 As New DataTable
						Dim ds1 As New DataSet
						dt1.TableName = "STATUS"
						ds1.Tables.Add(dt1)

						vsobj.FillDatasetAny("VitalSigns", "vitalsigns", strSQL, ds1, "STATUS")
						intTotalRowCount = Convert.ToDouble(dt1.Rows.Count)
					Catch ex As Exception
						WriteAuditEntry(Now.ToString & "  Error counting all the rows in the status table" & ex.ToString)
					End Try

					Try
						'str1SQL = "Select COUNT (Distinct  Status) from Status WHERE Type = 'Domino' AND Status <> 'Scanning'"
						strSQL = "SELECT Count(*) AS N FROM (SELECT DISTINCT Status FROM status st inner join Servers srv on srv.ServerName = st.Name inner join DominoServers ds on ds.ServerID = srv.id and ds.Enabled=1 WHERE st.Type = 'Domino' AND st.Status NOT IN ('Scanning', 'Not Scanned', 'Disabled','Insufficient Licenses') ) AS T "
					...
					strSQL = "SELECT COUNT(*) N FROM status st inner join Servers srv on srv.ServerName = st.Name inner join DominoServers ds on ds.ServerID = srv.id and ds.Enabled=1 AND "
					strSQL += " st.Type = 'Domino' AND st.Status NOT IN ('Scanning', 'Not Scanned', 'Disabled','Insufficient Licenses') AND LastUpdate  <= DATEADD(n,-45,GETDate())"


					Try
						intResult = Convert.ToDouble(vsobj.ExecuteScalarAny("VitalSigns", "Status", strSQL))
						WriteAuditEntry(Now.ToString & " There are  " & intResult & " Domino servers with status older than 45 minutes.")
					Catch ex As Exception
						intResult = 0  ' zero is good
					End Try


					'06/03/2014 List of servers with status older than 30 minutes
					strSQL = "SELECT Name FROM status st inner join Servers srv on srv.ServerName = st.Name inner join DominoServers ds on ds.ServerID = srv.id and ds.Enabled=1 "
					strSQL += " WHERE st.Type = 'Domino' AND st.Status NOT IN ('Scanning', 'Not Scanned', 'Disabled','Insufficient Licenses') AND LastUpdate  <= DATEADD(n,-45,GETDate())"
					Dim Servername As String
					Servername = ""

					Dim myDataSet As New Data.DataSet
					Dim myTable As New Data.DataTable
					myTable.TableName = "Status"
					myDataSet.Tables.Add(myTable)


					Try
						vsobj.FillDatasetAny("VitalSigns", "vitalsigns", strSQL, myDataSet, "Status")
					Catch ex As Exception
						WriteAuditEntry("Exception = " & ex.ToString())
						WriteAuditEntry(Now.ToString & " Error Accessing the Status Table " & ex.Message & strSQL)
					End Try

					Dim myView As New Data.DataView(myDataSet.Tables("Status"))
					...
					Try
						For Each drv In myView
							Try
								...
								Servername = Servername + drv("Name") + ","
							Catch ex As Exception
							End Try
						Next
					...
					Catch ex As Exception
						...
						Results = 0	 ' zero is good
					End Try

					' -------------------- START OF NEXT SCAN DID NOT REACH ON TIME. --------------------------


					Dim vsobject As New VSFramework.VSAdaptor
					Dim DsSettings As New Data.DataSet
					Dim Svalue As String = ""
					'Dim Min As String = "Select svalue from settings where sname = 'monitoringDelay'"
					Try
						strSQL = "Select svalue from Settings Where sname='MonitoringDelay'"
						vsobject.FillDatasetAny("VitalSigns", "None", strSQL, DsSettings, "Settings")
					...
					If Svalue > 0 Then
						'strSQLSelect = "Select Name, NextScan, Datediff(minute,nextscan,GETDate()) delayedBy,Type  from Status where Status <> 'Not Scanned' AND Status <> 'Insufficient Licenses'"
						strSQLSelect = "Select st.Name, st.NextScan, Datediff(minute,st.nextscan,GETDate()) delayedBy,st.Type FROM status st inner join Servers srv on srv.ServerName = st.Name "
						strSQLSelect += "inner join DominoServers ds on ds.ServerID = srv.id and ds.Enabled=1 WHERE st.Type = 'Domino' AND st.Status NOT IN ('Not Scanned','Insufficient Licenses') "
						'Dim StrSQLCount As String = "Select Count(*) from status where DateDiff(Minute,NextScan,GetDate()) > 2"
						...
						Try
							vsobj.FillDatasetAny("VitalSigns", "vitalsigns", strSQLSelect, myDataSet, "Status")
						Catch ex As Exception
							WriteAuditEntry("Exception = " & ex.ToString())
							WriteAuditEntry(Now.ToString & " Error Accessing the Status Table " & ex.Message & strSQLSelect)
						End Try
						Dim myView1 As New Data.DataView(myDataSet.Tables("Status"))
						Dim drv1 As DataRowView
						Dim myAlert As New AlertLibrary.Alertdll
						Dim type As String = ""
						Try
							For Each drv1 In myView1
								Try
									Dim delayedBy As Int32 = Convert.ToInt32(drv1("delayedBy"))
									Dim delaySetting As Int32 = Convert.ToInt32(Svalue)
									Servers = drv1("Name")
									type = drv1("Type")
									'Dim SummaryData As New Data.DataTable
									If delayedBy >= delaySetting Then
										myAlert.QueueAlert(type, Servers, "Monitoring Delay", Servers & " did not scan after next scan time of " & drv1("NextScan") & "", "Location")
									Else
										myAlert.ResetAlert(type, Servers, "Monitoring Delay", "Location")
									End If

								Catch ex As Exception

								End Try
								WriteAuditEntry(Now.ToString & " Servers which are not scanned after Next Scan time and the Server(s) is/are " & Servers & "")
							Next
					...
					' Case where the last update was more than 5 mins then lets restat the process. 
					' Still TO BE Implemented 
					' 1. Get Max scan interval based on business or non-business hours. 
					'strSQL = "SELECT ISNULL(DATEDIFF(MI,MAX(LastUpdate),GETDATE()),0) DiffInMin FROM [VitalSigns].[dbo].[Status] WHERE Type = 'Domino' AND  Status <> 'Not Scanned' AND Status <> 'Disabled' AND Status <> 'Insufficient Licenses'"
					strSQL = "SELECT ISNULL(DATEDIFF(MI,MAX(LastUpdate),GETDATE()),0) DiffInMin FROM status st inner join Servers srv on srv.ServerName = st.Name inner join DominoServers ds on ds.ServerID = srv.id and ds.Enabled=1 "
					strSQL += " WHERE st.Type = 'Domino' AND st.Status NOT IN ('Not Scanned', 'Disabled','Insufficient Licenses')"

					Try
						intResult = Convert.ToDouble(vsobj.ExecuteScalarAny("VitalSigns", "Status", strSQL))
						WriteAuditEntry(Now.ToString & " VitalSignsPlusDomino Monitoring Service was last updated " & intResult & " minutes ago. ")
					Catch ex As Exception
						intResult = 0  ' zero is good
					End Try
					...
					Try
						If ServiceRunning(VitalSignsPlusDomino) = True And ServiceStatus(VitalSignsPlusDomino) <> "Start Pending" And intResult > 0 Then
							Try
								'5/21/15 WS Commented out due to change in buisness hours.  Ticket # 
								'5/29/15 WS Changed VSPLUS-1789
								strSQL = "exec GetMaxDominoScanIntervalByHours"

								Dim maxScanInterval As Double = 0
								maxScanInterval = Convert.ToDouble(vsobj.ExecuteScalarAny("VitalSigns", "VitalSigns", strSQL))
								'WriteAuditEntry(Now.ToString & " The Max Scan Interval is " & maxScanInterval)
								If intResult > (maxScanInterval * 2) Then
									If DateTime.Now.Subtract(dominoKilled).Minutes > 15 Then
										dominoKilled = DateTime.Now
										'#2
					...
					'strSQLTotal = "SELECT COUNT(*) Total FROM STATUS WHERE Type = 'Domino' AND Status <> 'Not Scanned' AND Status <> 'Disabled' AND "
					'strSQLTotal += "Status <> 'Insufficient Licenses' "
					strSQLTotal = "Select COUNT(*) TOTAL FROM status st inner join Servers srv on srv.ServerName = st.Name inner join DominoServers ds on ds.ServerID = srv.id and ds.Enabled=1 "
					strSQLTotal += " WHERE st.Type = 'Domino' AND st.Status NOT IN ('Not Scanned', 'Disabled', 'Insufficient Licenses')"


					Dim strSQLTotalTelnet As String
					'strSQLTotalTelnet = "SELECT COUNT(*) TotalTelnet FROM STATUS WHERE Type = 'Domino' AND Status <> 'Not Scanned' AND Status <> 'Disabled' AND "
					'strSQLTotalTelnet += "Status <> 'Insufficient Licenses' and Description like '%via telnet%' "
					strSQLTotalTelnet = "SELECT COUNT(*) TotalTelnet FROM status st inner join Servers srv on srv.ServerName = st.Name inner join DominoServers ds on ds.ServerID = srv.id and ds.Enabled=1 "
					strSQLTotalTelnet += " WHERE st.Type = 'Domino' AND st.Status NOT IN ('Not Scanned', 'Disabled','Insufficient Licenses') AND st.Description like '%via telnet%'"
					...
					Try
						...
						intResult = Convert.ToDouble(vsobj.ExecuteScalarAny("VitalSigns", "Total", strSQLTotal))
						'WriteAuditEntry(Now.ToString & " There are  " & intResult & " Total Domino Servers in Monitoring")


						intResultTotalTelnet = Convert.ToDouble(vsobj.ExecuteScalarAny("VitalSigns", "TotalTelnet", strSQLTotalTelnet))
					...
					Try
						If ServiceRunning(VitalSignsPlusDomino) = True And ServiceStatus(VitalSignsPlusDomino) <> "Start Pending" Then
							strSQL = "select (case when nd.Value > s.svalue then 'True' else 'False' end) TimeToReset from Settings s inner join " & _
							 "NodeDetails nd on nd.Name = 'Domino Thread Killed Counter' and s.sname = 'Domino Thread Killed Counter Max' and nd.NodeId=(select ID from Nodes where Name=' " & _
							 System.Configuration.ConfigurationManager.AppSettings("VSNodeName").ToString() & "')"
							Dim result As String = ""
							Try
								result = vsobj.ExecuteScalarAny("VitalSigns", "TotalTelnet", strSQL).ToString()
							Catch ex As Exception
								result = "False"
							End Try
							...
							If Date.Now.Subtract(dominoKilled).Minutes > 15 And result = "True" Then
								Try
	...
	Protected Sub SendPulse()
		...
		'removes extras .0's on ServicesVersion until the length matches that of the db version
		'Try
		'	Dim numOfDecimalsForEXE As Integer = productVersion.Length - productVersion.Replace(".", "").Length
		'          Dim numOfDecimalsForDB As Integer = dbVersion.Length - dbVersion.Replace(".", "").Length
		'          Dim numofDecimalsForDLL As Integer = VSWebVersion.Length - VSWebVersion.Replace(".", "").Length
		'          If productVersion.Length = 0 Or dbVersion.Length = 0 Then
		'		Exit Try
		'          End If

		'          If VSWebVersion.Length = 0 Or dbVersion.Length = 0 Then
		'              Exit Try
		'          End If

		'	'we want it to be the same number as the number in table VSMANAGEMENT for value VS_VERSION if there are trailing .0's
		'	If numOfDecimalsForDB > numOfDecimalsForEXE Then
		'		While numOfDecimalsForDB > numOfDecimalsForEXE
		'			productVersion += ".0"
		'			numOfDecimalsForEXE = numOfDecimalsForEXE + 1
		'		End While
		'	ElseIf numOfDecimalsForDB < numOfDecimalsForEXE Then
		'		While numOfDecimalsForDB < numOfDecimalsForEXE
		'			If productVersion.Substring(productVersion.Length - 2) = ".0" Then
		'				productVersion = productVersion.Substring(0, productVersion.Length - 2)
		'			Else
		'				Exit While
		'			End If
		'			numOfDecimalsForEXE = numOfDecimalsForEXE - 1
		...
		'          If numOfDecimalsForDB > numofDecimalsForDLL Then
		'              While numOfDecimalsForDB > numofDecimalsForDLL
		'                  VSWebVersion += ".0"
		'                  numofDecimalsForDLL = numofDecimalsForDLL + 1
		'              End While
		'          ElseIf numOfDecimalsForDB < numofDecimalsForDLL Then
		'              While numOfDecimalsForDB < numofDecimalsForDLL
		'                  If VSWebVersion.Substring(VSWebVersion.Length - 2) = ".0" Then
		'                      VSWebVersion = VSWebVersion.Substring(0, VSWebVersion.Length - 2)
		'                  Else
		'                      Exit While
		'                  End If
		'                  numofDecimalsForDLL = numofDecimalsForDLL - 1
		...
		'          End If
		'Catch ex As Exception
		'	WriteAuditEntry(Now.ToString & " Error in SendPulse while changing productVersion.  Error: " & ex.Message.ToString())
		'End Try
		...
		Do
			...
			If NodeName <> "" Then
				Try
					...
					strSQL += " UPDATE Nodes SET NodeTime='" & Now.ToString & "'"


					If (hostname <> "") Then
						strSQL += ", HostName='" & hostname & "'"
					End If
					If (productVersion <> "") Then
						strSQL += ", Version='" & productVersion & "'"
					End If
					strSQL += " WHERE Name='" & NodeName & "' "


					vsobj.ExecuteScalarAny("VitalSigns", "", strSQL)
				Catch ex As Exception
				...
				'    End Try
				'End If
				strSQL = "DELETE FROM NodeDetails WHERE NodeID=(SELECT ID FROM Nodes WHERE Name='" & NodeName & "') AND Name like 'VSService_%'"
				For i As Integer = 0 To serviceNames.Count - 1
					Try

						status = ServiceStatus(serviceNames.Item(i))
						strSQL += " INSERT INTO NodeDetails (NodeId, Name, Value) VALUES ((SELECT ID FROM Nodes WHERE Name='" & NodeName & "'),'" & friendlyNames.Item(i) & "', '" & status & "') "
					Catch ex As Exception
					...
					Try
						vsobj.ExecuteScalarAny("VitalSigns", "", strSQL)
					Catch ex As Exception
						WriteAuditEntry(Now.ToString & " Error in SendPulse while updating NodeDetails table.  Error: " & ex.Message.ToString())
					End Try
				Next
